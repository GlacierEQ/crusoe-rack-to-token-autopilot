"""Behavioral scaffold tests for RackToTokenAutopilot.
...
def test_allow_path_returns_digest() -> None:
    ...
    receipt = mech.evaluate(
        RackToTokenAutopilotRequest(subject_id="a", payload={"x": 1}, budget=1.0)
    )
...
def test_refuse_missing_subject() -> None:
    mech = RackToTokenAutopilot()
    receipt = mech.evaluate(RackToTokenAutopilotRequest(subject_id="  ", payload={}, budget=1.0))
...
def test_refuse_non_positive_budget() -> None:
    mech = RackToTokenAutopilot()
    receipt = mech.evaluate(RackToTokenAutopilotRequest(subject_id="a", payload={}, budget=0.0))
    assert receipt.decision is Decision.REFUSE
...
def test_different_payloads_different_digests() -> None:
    mech = RackToTokenAutopilot()
    a = mech.evaluate(RackToTokenAutopilotRequest(subject_id="a", payload={"n": 1}, budget=1.0))
    b = mech.evaluate(RackToTokenAutopilotRequest(subject_id="a", payload={"n": 2}, budget=1.0))
